...
def make_test(pconfig:cPlatformConfig, graph_show=True):
    ...
    if use_clone: lt_web_attack = LinuxTarget(pconfig.getSSHNetworkWebAttack().copy())
    print('Start tests...')
    for count_exp in range(1,int(pconfig.getCountOfTests())+1):
        ...
        print("\n===============\nTEST №", count_exp)
        #init machines
        attacker = cAttacker(pconfig, lt_attacker, count_exp, use_clone=use_clone)
        ...
        if use_clone: print("Using clone")
        ...
        if pconfig.getUseVideo() and count_exp == 1:
...
if __name__ == '__main__':
    ...
    args = parser.parse_args()
    pconfig = cPlatformConfig(args.config)
    attacker = cAttacker(pconfig, None, 1, use_clone=True)
    client = cClient(pconfig, None, 1)
    graph = Graph(pconfig=pconfig)
    graph.plot_method_graph(attacker.parse_out(), graph_show=True)
    '''
    '''
    parser = argparse.ArgumentParser()
    parser.add_argument("-c", "--config", required=True,
